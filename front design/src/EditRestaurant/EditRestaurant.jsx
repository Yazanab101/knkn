...
export default function EditRestaurant() {
  ...
  useEffect(() => {
    const fetchRestaurant = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5002/get-one-res/${resName}`, { headers });
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(`http://localhost:5002/update-restaurant/${resName}`, {
